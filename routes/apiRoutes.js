...
module.exports = function (app) {
  ...
  app.post("/api/friends", function (req, res) {
    ...
    for (var i = 0; i < friendSurveys.length; i++) {
      ...
      if (friendSurveys[i].name != userAnswers.name) {
        //(TODO: this exclusion would be less fallible if every user had a unique id..)
    ...
    function blurbGenerator(profile) {
      //a function that can be passed a profile and assemble a blurb of descriptive text about them based on their survey answers

      var descriptorArray = ["", "", "", "", "", "", "", "", "", ""]; //array of ten empty strings to be set by loop

      for (var i = 0; i < descriptorArray.length; i++) {
        switch (
          profile.scores[i] //look at their answer to each survey question
        ) {
          case 1:
      ...
      var blurb =
        //a blurb about the profile is built out of sentences assigned to vars by the switchcase statement above, with the cracks filled in by a few hardcoded words:
    ...
    console.log(matchingProfileName + " is your best match on Xenofriendr!");
